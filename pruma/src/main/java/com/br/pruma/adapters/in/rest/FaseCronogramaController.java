...
public class FaseCronogramaController {
    ...
    @Operation(summary = "Atualiza fase por ID")
    @PutMapping("/{id}")
    public ResponseEntity<FaseCronogramaResponseDTO> atualizar(@PathVariable Integer id,
                                                               @RequestBody @Valid FaseCronogramaRequestDTO dto) {
        return ResponseEntity.ok(service.update(id, dto));
    }

    @Operation(summary = "Deleta fase por ID")
    @DeleteMapping("/{id}")
