...
public class ProfissionalDeBaseController {
    ...
    @Operation(summary = "Cria novo profissional")
    @PostMapping
    ...
    @Operation(summary = "Atualiza profissional por ID")
    @PutMapping("/{id}")
    public ResponseEntity<ProfissionalDeBaseResponseDTO> atualizar(@PathVariable Integer id,
                                                                    @RequestBody @Valid ProfissionalDeBaseRequestDTO dto) {
        return ResponseEntity.ok(service.update(id, dto));
    }

    @Operation(summary = "Deleta profissional por ID")
    @DeleteMapping("/{id}")
