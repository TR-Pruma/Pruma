...
-- V25 criou tipo_usuario como VARCHAR(50) mas o modelo espera tipo_usuario_id INT (FK).
-- Todos os passos sao condicionais (idempotente).

SET FOREIGN_KEY_CHECKS = 0;

-- 1. Renomeia tipo_usuario VARCHAR -> tipo_usuario_id INT (so se tipo_usuario existir como VARCHAR
--    E tipo_usuario_id ainda nao existir - evita duplicate column)
...
SET @nao_tem_int  = (SELECT COUNT(*) FROM information_schema.columns
                     WHERE table_schema = DATABASE() AND table_name = 'auditoria'
                       AND column_name = 'tipo_usuario_id');
SET @s = IF(@tem_varchar > 0 AND @nao_tem_int = 0,
    'ALTER TABLE auditoria CHANGE COLUMN tipo_usuario tipo_usuario_id INT NULL',
    'SELECT 1');
PREPARE st FROM @s; EXECUTE st; DEALLOCATE PREPARE st;

-- 2. Remove coluna tipo_usuario VARCHAR caso ainda exista (independente do passo 1)
...
-- 3. Garante que tipo_usuario_id existe (caso nao tenha vindo de nenhum passo anterior)
SET @n = (SELECT COUNT(*) FROM information_schema.columns
          WHERE table_schema = DATABASE() AND table_name = 'auditoria'
            AND column_name = 'tipo_usuario_id');
SET @s = IF(@n = 0,
...
-- 4. Adiciona FK fk_auditoria_tipo_usuario (se nao existir)
